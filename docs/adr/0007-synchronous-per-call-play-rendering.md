...
*behind* that backlog — constant latency equal to the buffer depth — and short notes were easily
...
Chosen option: drop the render thread. `playFrame(VoiceFrame, VoiceFrame, VoiceFrame,
durationSeconds)` now writes synchronously, for exactly `durationSeconds`, on the calling thread —
no separate thread, no idle silence ever queued. The line's own backpressure (`write()` blocking
once the buffer is full) then paces playback for free: `StatementExecutor`'s ~20ms pull loop needs
no cadence mechanism of its own beyond a fallback sleep for the no-device (headless/test) case,
where `write()` returns instantly.
