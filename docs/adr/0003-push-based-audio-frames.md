...
`BREAK` could never fire and `PLAY` returned instantly in tests, unlike `BEEP`/`PAUSE`. Inverting
the design keeps all DSL parsing, timing, and BREAK-handling logic inside `StatementExecutor`,
mirroring `executeBeepStmt`'s chunked-sleep/BREAK-poll shape exactly.
...
- Neutral: this decision is about who _initiates_ each audio frame, not about buffering - the
