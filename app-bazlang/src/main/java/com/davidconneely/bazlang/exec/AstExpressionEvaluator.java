 ...
 * {@code numResult}/{@code strResult} side fields, unlike the ANTLR-visitor-based {@link
 * ExpressionEvaluator} this will eventually replace (see {@code localonly-plan-CUSTOM-AST.md},
 * decision 10: renamed at cutover).
 *
 * <p>Not wired into the live interpreter yet (Phase 1 of that plan). Same collaborators as {@link
 * ExpressionEvaluator} so it can be dropped in unchanged at cutover.
 */
public class AstExpressionEvaluator {
  ...
  private BStr evalStrFuncCall(StrExpr.StrFuncCall call) {
    ...
    return switch (call.kind()) {
      ...
      case STR_STR -> BStr.fromJavaString(ExpressionEvaluator.formatNum(argNum(args, 0)));
      case UCHR_STR -> {
        final int code = (int) argNum(args, 0);
  ...
  private Object evaluateFnCall(String name, List<Expr> callArgs) {
    ...
    try {
      final int lineNumber = state.currentLineLabel();
      if (name.endsWith("$")) {
        return evalStr(AstLowering.lowerStr(def.body().strExpr(), lineNumber));
      } else {
        return evalNum(AstLowering.lowerNum(def.body().numExpr(), lineNumber));
      }
